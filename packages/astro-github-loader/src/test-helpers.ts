...
export function createMockContext() {
  ...
  return {
    store: {
      set: (entry: any) => { store.set(entry.id, entry); return entry; },
    ...
    entryTypes: new Map([
      ['.md', {
        getEntryInfo: async ({ contents }: { contents: string; fileUrl: URL }) => ({
          body: contents,
          data: {},
        }),
      }],
...
export const MOCK_COMMIT = {
  ...
  commit: {
    ...
    committer: { name: 'Test Committer', email: 'test@example.com', date: '2024-01-01T00:00:00Z' },
  },
};
...
export const MOCK_TREE_DATA = {
  ...
  tree: [
    { path: 'docs/algokit.md', mode: '100644', type: 'blob', sha: 'file1sha', size: 1234, url: 'https://api.github.com/repos/test/repo/git/blobs/file1sha' },
    { path: 'docs/features', mode: '040000', type: 'tree', sha: 'dir1sha', url: 'https://api.github.com/repos/test/repo/git/trees/dir1sha' },
    { path: 'docs/features/accounts.md', mode: '100644', type: 'blob', sha: 'file2sha', size: 2345, url: 'https://api.github.com/repos/test/repo/git/blobs/file2sha' },
    { path: 'docs/features/tasks.md', mode: '100644', type: 'blob', sha: 'file3sha', size: 3456, url: 'https://api.github.com/repos/test/repo/git/blobs/file3sha' },
    { path: 'docs/features/generate.md', mode: '100644', type: 'blob', sha: 'file4sha', size: 4567, url: 'https://api.github.com/repos/test/repo/git/blobs/file4sha' },
    { path: 'docs/cli/index.md', mode: '100644', type: 'blob', sha: 'file5sha', size: 5678, url: 'https://api.github.com/repos/test/repo/git/blobs/file5sha' },
    { path: 'README.md', mode: '100644', type: 'blob', sha: 'file6sha', size: 678, url: 'https://api.github.com/repos/test/repo/git/blobs/file6sha' },
    { path: 'package.json', mode: '100644', type: 'blob', sha: 'file7sha', size: 789, url: 'https://api.github.com/repos/test/repo/git/blobs/file7sha' },
  ],
  truncated: false,
};
...
export function createMockOctokit(options?: {
  ...
  const listCommitsSpy = vi.spyOn(octokit.rest.repos, 'listCommits')
    .mockResolvedValue({ data: [commit], status: 200, url: '', headers: {} } as any);

  const getTreeSpy = vi.spyOn(octokit.rest.git, 'getTree')
    .mockResolvedValue({ data: tree, status: 200, url: '', headers: {} } as any);

  const getContentSpy = vi.spyOn(octokit.rest.repos, 'getContent')
 ...
 */
export function mockFetch(content: string = '# Test Content\n\nThis is test markdown content.') {
