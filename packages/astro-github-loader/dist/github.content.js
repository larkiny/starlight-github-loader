 ...
 * @internal
 */
async function processAssets(content, options, octokit, signal) {
 ...
 * @internal
 */
export async function syncEntry(context, { url, editUrl }, options, octokit, init = {}) {
 ...
 * @internal
 */
export async function toCollectionEntry({ context, octokit, options, signal, }) {
    ...
    if (!Array.isArray(data)) {
        ...
        switch (data.type) {
            // Return
            case "file":
                return await syncEntry(context, { url: data.download_url, editUrl: data.url }, { ...options, path, ref }, octokit, { signal });
            default:
                throw new Error("Invalid type");
        }
    }
    // Directory listing
    const promises = data.map(({ type, path, download_url, url }) => {
        switch (type) {
            // Recurse
            case "dir":
            ...
            // Return
            case "file":
                return syncEntry(context, { url: download_url, editUrl: url }, { ...options, path, ref }, octokit, { signal });
            default:
                throw new Error("Invalid type");
        }
    });
